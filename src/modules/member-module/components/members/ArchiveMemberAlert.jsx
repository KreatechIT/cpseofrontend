...
import { archiveMember } from "../../services/organisationService";

const ArchiveMemberAlert = () => {
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const memberInfo = useSelector((state) => state.dialog.props);

  const handleConfirm = () => {
    archiveMember(user?.organisation_id, memberInfo?.id, dispatch).then(() => {
...
export default ArchiveMemberAlert;
