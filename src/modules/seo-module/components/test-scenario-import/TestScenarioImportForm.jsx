...
const TestScenarioImportForm = () => {
  ...
  // Fetch projects (same as conversion)
  useEffect(() => {
    ...
    const importPromises = importedData.map(async (row) => {
      try {
        const payload = {
          ...
          test_scenario: getValue(row, "Test Scenario") || "",
          sub_project: getValue(row, "Sub Project") || "",
          unique_domain: getValue(row, "Unique Domain") || "",
          page_url: getValue(row, "Page/URL") || "",
          hypothesis: getValue(row, "Hypothesis") || "",
          expected_result: getValue(row, "Expected Result") || "",
          start_date: formatDateForAPI(getValue(row, "Start Date")),
        ...
        await importTestScenarioData(payload);
        successCount++;
      } catch (error) {
        errorCount++;
      }
  ...
  return (
    <div className="space-y-8 mt-3">
      ...
      <div className="space-y-4">
        <div>
          <Label className="mb-3">File Title (optional)</Label>
      ...
      <div className="flex justify-end">
        <Button onClick={handleImport} disabled={loading || !importedData || !selectedProject}>
          {loading ? "Uploading..." : "Upload Test Scenarios"}
        </Button>
      </div>
      ...
      {importedData && (
        <div className="border p-6 rounded-lg space-y-6 bg-gray-50">
          ...
          <TestScenarioImportTable data={importedData} columns={columns} />
        </div>
      )}
    </div>
  );
...
export default TestScenarioImportForm;